#!/usr/bin/env python3
"""Run protocol-31: source-only paired-view candidate role-switch audit.

The audit is deliberately not a ranker training entrypoint.  It locks the
unified high-resolution epoch-3 candidate checkpoint, reproduces the clean
source-validation baseline, then re-runs DINO/RPN/ROI for two deterministic
label-preserving image views on source train/train_sim.  Target data is never
opened and no checkpoint can be selected or overwritten.
"""
...
AUDIT_NAME = 'DINO S7 Paired-view Candidate Role-switch Support Audit'
LOCKED_EPOCH = 3
...
def parse_args():
    ...
    parser.add_argument('--min-gain-frames', type=int, default=32)
    parser.add_argument('--min-gain-sequences', type=int, default=3)
    parser.add_argument('--max-sequence-fraction', type=float, default=0.50)
...
def validate_args(args):
    if args.seed != 0:
        raise ValueError('Paired-view role-switch audit requires --seed 0')
    if args.dinov2_model != 'dinov2_vitl14':
        raise ValueError('Paired-view role-switch audit requires dinov2_vitl14')
    ...
    if args.min_gain_frames <= 0 or args.min_gain_sequences <= 0:
        raise ValueError('Paired-view support floors must be positive')
    if not 0.0 < args.max_sequence_fraction <= 1.0:
        raise ValueError('--max-sequence-fraction must be in (0, 1]')
...
def build_locked_labeller_argv(args) -> List[str]:
    return [
        ...
        '--source-paired-view-role-switch-support-audit',
        '--source-paired-view-source-result-json', args.source_result_json,
        '--source-paired-view-min-gain-frames', str(args.min_gain_frames),
        '--source-paired-view-min-gain-sequences', str(args.min_gain_sequences),
        '--source-paired-view-max-sequence-fraction',
        str(args.max_sequence_fraction),
