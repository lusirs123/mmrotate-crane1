#!/usr/bin/env python3
"""Run the locked source-only smooth-geometry rank-support audit.

The input JSON is the unified high-resolution source-only training result
(protocol 23), not the later margin-audit output.  The wrapper locks the
epoch-3 checkpoint and source-small sampling, then invokes the common labeller
in read-only mode.  No target annotation is opened.
"""
...
AUDIT_NAME = 'DINO S7 Smooth Geometry Rank-Support Audit'
LOCKED_EPOCH = 3
...
def parse_args():
    ...
    parser.add_argument(
        '--source-result-json', required=True,
        help=('Unified high-resolution source-only training result JSON, '
              'protocol 23. Do not pass margin_audit_result.json here.'))
...
def validate_args(args):
    if args.seed != 0:
        raise ValueError('Smooth geometry audit requires --seed 0')
    if args.dinov2_model != 'dinov2_vitl14':
        raise ValueError('Smooth geometry audit requires dinov2_vitl14')
...
def build_locked_labeller_argv(args) -> List[str]:
    return [
        ...
        '--source-smooth-geometry-rank-support-audit',
        '--source-smooth-geometry-source-result-json',
        args.source_result_json,
        '--source-smooth-geometry-min-gain-domains',
