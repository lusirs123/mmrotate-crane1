#!/usr/bin/env python3
"""One-shot fixed target-dev audit for a source-selected stage-3 student.

The phase-2 fixed-target audit intentionally accepts only the phase-2
``s7_temporal_association`` checkpoint.  This entry keeps that protocol
unchanged and provides the corresponding locked evaluator for a positive
stage-3 ``s7_temporal_student`` checkpoint.
"""
...
AUDIT_NAME = 'DINO S7 Temporal Student Fixed Three-Slice Target-Dev Audit V1'
PROTOCOL_VERSION = 1
EXPECTED_TEACHER_EPOCH = 4
SOURCE_GATE = dict(
    min_full_top1=688,
    min_small_top1=311,
    max_full_mcml=3,
    max_small_mcml=3,
    max_lost_correct=0)
TARGET_GATES = fixed_target.TARGET_GATES


def parse_args():
    """Reuse the immutable target-slice command-line surface."""
    return fixed_target.parse_args()
...
def _selected_source_history(result: Dict) -> Dict:
    ...
    if len(matches) != 1:
        raise ValueError(
            'Expected one stage-3 source history row for best epoch {}, '
            'found {}'.format(best_epoch, len(matches)))
    return matches[0]


def strict_stage3_source_gate(result: Dict) -> Dict:
    """Revalidate a positive stage-3 source-only result before target access."""
    source = result.get('source', {})
    best_epoch = int(source.get('best_epoch', -1))
    ...
    student_protocol = protocol.get('s7_temporal_student') or {}
    reproduction = source.get('initial_teacher_reproduction_gate') or {}
    baseline_correct = int(retention.get('baseline_correct_count', -1))
    retained_correct = int(retention.get('retained_correct_count', -2))
    checks = dict(
        protocol_version_current=int(result.get('protocol_version', -1)) >= 22,
        positive_best_epoch=best_epoch > 0,
        ...
        full_top1_absolute=(
            int(full.get('top1_hits', -1)) >= SOURCE_GATE['min_full_top1']),
        small_top1_absolute=(
            int(small.get('top1_hits', -1)) >= SOURCE_GATE['min_small_top1']),
        full_mcml_absolute=(
        ...
        student_training_protocol=(
            isolation.get('train_components') == 's7_temporal_student'
        ...
        teacher_reproduction=(
            reproduction.get('passed') is True),
...
def configure_locked_stage3_model(args):
    """Install the exact stage-3 student architecture."""
    fixed_target.configure_locked_model(args)
    args.train_components = 's7_temporal_student'
    args.s7_temporal_student = True
    args.s7_student_base_epoch = EXPECTED_TEACHER_EPOCH
    args.s7_student_hidden = 128
    args.s7_student_quality_loss_weight = 1.0
    args.s7_student_relative_loss_weight = 0.5
    args.s7_student_distillation_weight = 1.0
    args.s7_student_distillation_temperature = 1.0
    args.s7_student_small_loss_weight = 2.0
...
def candidate_checkpoint_gate(payload: Dict, source_gate: Dict,
    ...
    temporal_protocol = training.get('s7_temporal_association') or {}
    ...
    checks.update(
        checkpoint_positive_and_selected=(
            ...
            and int(payload.get('epoch', -1)) == int(
                payload.get('best_epoch', -2))
            and int(payload.get('best_epoch', -1)) == int(
                source_gate.get('best_epoch', -2))),
...
def _validate_args(args):
    if args.seed != 0:
        raise ValueError('The fixed stage-3 target protocol requires --seed 0')
    ...
    args.strict_source_gate = strict_stage3_source_gate(args.source_result)
    if not args.strict_source_gate['passed']:
        failed = sorted(
            name for name, passed in args.strict_source_gate['checks'].items()
            if not passed)
        raise ValueError('Strict stage-3 source gate failed: {}'.format(
            ', '.join(failed)))
    selected = args.strict_source_gate.get('selected_checkpoint')
    if not selected or os.path.realpath(selected) != os.path.realpath(
            args.candidate_checkpoint):
        raise ValueError(
            'Candidate checkpoint must equal stage-3 source_selected_checkpoint')
    configure_locked_stage3_model(args)
    args.parsed_target_slices = [
        dict(fixed_target.EXPECTED_TARGET_SLICES[name])
...
def main():
    ...
    if not candidate_heads.s7_inference_enabled():
        raise RuntimeError('Stage-3 candidate did not enable S7 inference')
    ...
    for spec in args.parsed_target_slices:
        ...
        candidate_rows = labeller.evaluate_records(
            ...
            role='target_dev_diagnosis_only')
        slices[spec['name']] = fixed_target.fixed_slice_result(
    ...
    decision = (
        'S7_TEMPORAL_STUDENT_FIXED_TARGET_DEV_PASS'
        if passed else
        'S7_TEMPORAL_STUDENT_FIXED_TARGET_DEV_FAIL_KEEP_SOURCE_MODEL')
    payload = dict(
        audit=AUDIT_NAME, protocol_version=PROTOCOL_VERSION,
        decision=decision, eligible_for_deployment=False,
        ...
        checkpoints=dict(
            ...
            candidate_best_epoch=int(args.strict_source_gate['best_epoch'])),
        protocol=dict(
            fixed_one_shot=True,
            all_frames_unified_policy=True,
            candidate_mode='s7_temporal_student_source_only',
    ...
    for name in ('seq02_far', 'seq02_dark', 'seq03_small'):
        result = slices[name]
        print(
            '[stage3-fixed-target] {} baseline={}/{} candidate={}/{} '
