#!/usr/bin/env python3
"""Run the locked source-only native-relative-risk support audit.

The protocol-29 result is accepted only as provenance for its rejected epoch-0
fallback checkpoint and its observed zero-effective-penalty failure.  This
wrapper performs one frozen source-train pass; it never trains or opens target
annotations.
"""
...
AUDIT_NAME = 'DINO S7 Native-relative Risk Score-gap Support Audit'
...
def parse_args():
    ...
    parser.add_argument('--smooth-geometry-support-result-json', required=True)
...
def validate_args(args):
    if args.seed != 0:
        raise ValueError('Native-relative risk audit requires --seed 0')
    if args.dinov2_model != 'dinov2_vitl14':
        raise ValueError('Native-relative risk audit requires dinov2_vitl14')
    for name in ('source_result_json', 'eval_only_checkpoint',
                 'smooth_geometry_support_result_json', 'dinov2_checkpoint'):
    ...
    labeller.load_native_relative_risk_support_audit_spec(
...
def build_locked_labeller_argv(args) -> List[str]:
    return [
        ...
        args.smooth_geometry_support_result_json,
        '--s7-highres-base-epoch', '3', '--s7-highres-hidden', '32',
        '--s7-highres-channels', '32', '--s7-highres-max-candidates', '32',
        '--s7-highres-score-weight', '1.0', '--s7-highres-rank-margin', '0.25',
        ...
        '--s7-highres-smooth-geometry-metric', 'sym_kld',
        '--s7-highres-smooth-geometry-loss-weight', '0.25',
        '--s7-highres-smooth-geometry-min-gap', '0.05',
        '--s7-highres-smooth-geometry-max-pairs', '64',
        ...
        '--source-native-relative-risk-support-audit',
        '--source-native-relative-risk-source-result-json',
