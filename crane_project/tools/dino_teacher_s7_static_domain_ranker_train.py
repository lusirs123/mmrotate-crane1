#!/usr/bin/env python3
"""Locked one-shot source-only static domain-generalized S7 ranker."""
...
TRAINING_NAME = 'DINO S7 Static Domain-Generalized Candidate Ranker V1'
...
def validate_args(args):
    if args.seed != 0:
        raise ValueError('Static ranker training requires --seed 0')
    if args.dinov2_model != 'dinov2_vitl14':
        raise ValueError('Static ranker training requires dinov2_vitl14')
    ...
    if not selected or os.path.realpath(selected) != os.path.realpath(
        ...
        raise ValueError(
            'Static ranker init checkpoint must equal the phase-2 '
...
def build_locked_labeller_argv(args) -> List[str]:
    return [
        ...
        '--s7-static-domain-ranker',
        '--s7-static-base-epoch', str(EXPECTED_BASE_EPOCH),
        '--s7-static-hidden', '128',
        '--s7-static-quality-loss-weight', '1.0',
        '--s7-static-relative-loss-weight', '0.5',
        '--s7-static-relative-margin', '0.25',
        '--s7-static-relative-min-gap', '0.10',
        '--s7-static-relative-max-pairs', '128',
        '--s7-static-score-weight', '1.0',
        '--s7-static-rank-margin', '0.25',
        '--s7-static-retention-weight', '2.0',
        '--s7-static-gain-weight', '1.0',
        '--s7-static-prior-weight', '0.01',
        '--s7-static-max-candidates', '100',
        '--s7-static-aug-prob', '0.75',
        '--s7-static-aug-strength', '0.15',
        '--s7-static-teacher-result-json', args.source_result_json,
        '--s7-source-min-full-top1', '688',
        '--s7-source-min-small-top1', '311',
        '--s7-source-max-mcml', '3',
        '--train-components', 's7_static_domain_ranker',
