#!/usr/bin/env python3
"""Run protocol-33: source-only dense temporal separability audit.

The audit locks the unified high-resolution epoch-3 candidate checkpoint and
uses source GT only to label dense native/S7 candidates.  It measures whether
GT-matched candidates are more consistent than high-score hard negatives
across adjacent frames and deterministic label-preserving image views.  No
adapter is fitted, no checkpoint is selected, and target data is never opened.
"""
...
AUDIT_NAME = 'DINO S7 Dense Object-Centric Temporal Separability Audit'
LOCKED_EPOCH = 3
...
def parse_args():
    ...
    parser.add_argument('--min-pairs-per-sequence', type=int, default=32)
    parser.add_argument('--min-real-sequences', type=int, default=2)
    parser.add_argument('--min-sim-sequences', type=int, default=1)
    parser.add_argument('--min-success-fraction', type=float, default=0.60)
    parser.add_argument('--min-cosine-margin', type=float, default=0.02)
    parser.add_argument('--negative-riou-max', type=float, default=0.30)
...
def validate_args(args):
    if args.seed != 0:
        raise ValueError('Dense temporal audit requires --seed 0')
    if args.dinov2_model != 'dinov2_vitl14':
        raise ValueError('Dense temporal audit requires dinov2_vitl14')
    ...
    for name in ('min_pairs_per_sequence', 'min_real_sequences',
                 'min_sim_sequences', 'hard_negatives'):
        if int(getattr(args, name)) <= 0:
            raise ValueError('--{} must be positive'.format(
                name.replace('_', '-')))
    if not 0.5 < args.min_success_fraction <= 1.0:
        raise ValueError('--min-success-fraction must be in (0.5, 1]')
    if args.min_cosine_margin < 0.0:
        raise ValueError('--min-cosine-margin must be non-negative')
    if not 0.0 <= args.negative_riou_max < 0.5:
        raise ValueError('--negative-riou-max must be in [0, 0.5)')
    labeller.load_paired_view_role_switch_support_audit_spec(
        args.source_result_json, args.eval_only_checkpoint)
...
def build_locked_labeller_argv(args) -> List[str]:
    return [
        ...
        '--source-dense-temporal-separability-audit',
        '--source-dense-temporal-source-result-json', args.source_result_json,
        '--source-dense-temporal-min-pairs-per-sequence',
        str(args.min_pairs_per_sequence),
        '--source-dense-temporal-min-real-sequences',
        str(args.min_real_sequences),
        '--source-dense-temporal-min-sim-sequences',
        str(args.min_sim_sequences),
        '--source-dense-temporal-min-success-fraction',
        str(args.min_success_fraction),
        '--source-dense-temporal-min-cosine-margin',
        str(args.min_cosine_margin),
        '--source-dense-temporal-negative-riou-max',
        str(args.negative_riou_max),
        '--source-dense-temporal-hard-negatives', str(args.hard_negatives),
