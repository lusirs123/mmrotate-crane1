# =========================================================
# SymEOOD(K=1) — 以 EOOD(K=1) 为起点的三件套消融
# 学术定位：1 主头(SymEOODHead) + 1 辅助头(ATSS)，推理仅主头。
#
# 相对 crane_eood_k1.py：
#   - 主头从 RotatedEoodHead 替换为 SymEOODHead
#   - 分类损失替换为 SymNFLLoss
#   - 回归损失替换为 SymKLDLoss
#   - 正样本分配替换为 SymPOLAAssigner
#   - 保留 K=1 的单个 RotatedATSSHead 辅助监督
#
# 相对旧 crane_symeood_m1.py：
#   - 不再表示“无辅助头”的 M1
#   - 明确命名为 K=1 起点，避免和 EOOD 的 K 消融混淆
# =========================================================
...
# =========================================================
# 模型：SymEOOD(K=1) — 三件套主头 + 1 个 ATSS 辅助头
# =========================================================
model = dict(
    type='SymEOOD',
    backbone=dict(
    ...
    bbox_head=dict(
        ...
        # 创新点 1+2：SymNFLLoss
        # 空间感知对称归一化 Focal，用于替换 EOOD(K=1) 主头分类损失。
        loss_cls=dict(
            type='SymNFLLoss',
            use_sigmoid=True,
            gamma=2.0,
        ...
        # 创新点 1：SymKLDLoss
        # 对称 KL 高斯流形回归，用于替换 EOOD(K=1) 主头 L1/IoU 回归组合。
        loss_bbox=dict(
            type='SymKLDLoss',
            eps=1e-6,
            reduction='mean',
    ...
    train_cfg=dict(
        # 创新点 3：SymPOLAAssigner
        # 预测感知 O2M -> O2O 分配，与 SymNFLLoss 的温度退火保持同步。
        assigner=dict(
    ...
    # K=1 辅助监督：与 crane_symeood_m2.py 保持等价的单个 RotatedATSSHead。
    # 辅助头只参与训练，SymEOOD.simple_test() 推理期只调用主头。
    aux_bbox_head=[dict(
...
load_from = None
resume_from = None
work_dir = 'work_dirs/crane_symeood_k1'
