"""Three-window block-CV training view for the source-only E1 refiner.

Set ``SEQ11_CV_FOLD`` to 1, 2, or 3.  The selected window is excluded from
training and is embedded in the checkpoint evidence contract.  Fixed TEST is
absent and fold choice is never inferred from performance.
"""
...
_folds = {
    1: dict(train_count=49, val_count=10),
    2: dict(train_count=47, val_count=12),
    3: dict(train_count=48, val_count=11),
}
if cv_fold_id not in _folds:
    raise RuntimeError('SEQ11_CV_FOLD must be explicitly set to 1, 2, or 3')
_fold = _folds[cv_fold_id]
...
aux_train_split = (
    'extra_source_real_seq11_pilot_k1p9_cv_fold{}_train_v1'.format(
        cv_fold_id))
aux_val_split = (
    'extra_source_real_seq11_pilot_k1p9_cv_fold{}_val_v1'.format(
        cv_fold_id))
aux_train_audit = '{}/fold{}/train_all_lane_audit.json'.format(
    cv_root, cv_fold_id)
aux_val_audit = '{}/fold{}/val_all_lane_audit.json'.format(
    cv_root, cv_fold_id)
cv_manifest = (
...
aux_train_pipeline = _train_pipeline(
    aux_train_audit, _fold['train_count'], aux_train_split)
...
evidence_contract = dict(
    source_train_frames=2781 + _fold['train_count'],
    original_source_train_frames=2781,
    auxiliary_source_frames=59,
    auxiliary_source_train_frames=_fold['train_count'],
    auxiliary_source_val_frames=_fold['val_count'],
    auxiliary_source_sequence='real_seq11',
    auxiliary_cv_protocol=(
        'real_seq11_auxiliary_three_window_block_cv_v1'),
    auxiliary_cv_fold=cv_fold_id,
    auxiliary_cv_manifest=cv_manifest,
    auxiliary_cv_manifest_sha256=cv_manifest_sha256,
...
work_dir = (
    ...
    'source_v3_seq11_blockcv_fold{}_seed3407'.format(cv_fold_id))
