...
- 用户已在另一个对话开展 DINO 问题复查；本对话等待其结果，不重复启动复查或新实验。
- 接收复查结果后，先记录对应配置、checkpoint、数据范围、证据路径，以及对历史判断的确认或修正，再确定下一项实验。
- 暂不修改模型、训练参数、阈值或数据划分，不启动训练、推理或压缩。此前关于排序瓶颈、监督支持与轻量化路线的分析是研究依据，不是新实验结果。
- 三段固定 TEST 困难片段已参与历史开发诊断；source-only 训练不使其重新成为未见测试集。后续实验需明确训练、选择与报告各自使用的数据。
